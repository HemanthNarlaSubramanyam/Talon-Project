...
/* One-time: make sure staging tables exist */
IF OBJECT_ID('bronze.customer_sessions_stage','U') IS NULL
CREATE TABLE bronze.customer_sessions_stage (
  session_id NVARCHAR(64), created NVARCHAR(64), closedAt NVARCHAR(64), cancelledAt NVARCHAR(64),
  state NVARCHAR(40), total_usd NVARCHAR(50), customer_profile_fk NVARCHAR(64),
  store_integration_id NVARCHAR(64), number_of_cart_items NVARCHAR(50), channel NVARCHAR(64)
);

IF OBJECT_ID('bronze.effects_stage','U') IS NULL
CREATE TABLE bronze.effects_stage (
  effect_id NVARCHAR(64), session_fk NVARCHAR(64), customer_profile_fk NVARCHAR(64),
  created_ts NVARCHAR(64), effect_type NVARCHAR(128), name NVARCHAR(MAX), value NVARCHAR(MAX)
);
GO
...
BEGIN
  ...
  ---------------------------------------------------------------------------
  -- A) customer_sessions.csv  (edit path if needed)
  ---------------------------------------------------------------------------
  ...
  WITH (
    ...
    FIELDQUOTE='"',
    FIRSTROW=2,
    ROWTERMINATOR='0x0a',   -- change to '0x0d0a' if Windows line endings
    TABLOCK,
    KEEPNULLS,
    MAXERRORS=0
  );
  ...
  ---------------------------------------------------------------------------
  -- B) effects.csv  (edit path if needed)
  ---------------------------------------------------------------------------
